  ...
  const features = [
    {
      ...
      desc: "Safe for sensitive skin",
    },
    {
      icon: Leaf,
      title: "Biodegradable Pads",
      desc: "Eco-friendly disposal",
    },
    {
      icon: Sparkles,
      title: "Ultra-Soft Top Sheet",
      desc: "Cloud-like comfort",
    },
    ...
    <div className="space-y-8">
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="lg:sticky lg:top-24">
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            {features.slice(0, 3).map((feature) => (
              <div
                key={feature.title}
                className="text-center p-4 rounded-xl bg-kumfora-cream"
              >
                ...
                <p className="text-caption font-medium text-kumfora-charcoal">
                  {feature.title}
                </p>
              </div>
            ))}
        ...
        <div className="space-y-6">
          ...
          {/* Title */}
          <h1 className="text-display-md font-display font-medium text-kumfora-charcoal">
            {product.name}
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
                  ...
                  <Star
                    ...
                    className={cn(
                      ...
                      i < Math.floor(product.rating)
                        ? "fill-kumfora-coral text-kumfora-coral"
              ...
              <span className="text-body font-medium text-kumfora-charcoal">
                {product.rating}
              </span>
            ...
            {discount > 0 && (
              <Badge variant="danger" className="text-body-sm">
                {discount}% OFF
              </Badge>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-baseline gap-4">
            ...
            {product.originalPrice && (
              <Badge variant="danger" className="text-body-sm ml-2">
                {discount}% OFF
              </Badge>
            )}
          </div>
          ...
          <div className="space-y-3">
            {features.map((feature) => (
              <div key={feature.title} className="flex gap-3">
                ...
                <div>
                  <p className="text-body-sm font-medium text-kumfora-charcoal">
                    {feature.title}
                  </p>
      ...
      <div className="border-t border-kumfora-lightGray/50 pt-8">
        <div
          className="flex gap-1 bg-kumfora-cream rounded-xl p-1 mb-8 overflow-x-auto scrollbar-hide"
          role="tablist"
...
function ProductDetailsContent({ product }: { product: Product }) {
  return (
    <div className="prose prose-kumfora max-w-none space-y-6">
      ...
      <p className="text-body text-kumfora-slate leading-relaxed">
        Designed specifically for young girls, our pads combine superior
        absorption with ultra-soft comfort. Each pad features leak-guard
        barriers and a breathable design that keeps you feeling fresh and
        confident all day long. The biodegradable wrapper makes disposal easy
        and eco-friendly.
      </p>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {Object.entries(product.specifications).map(([key, value]) => (
          <div
            key={key}
            className="flex flex-col sm:flex-row items-start sm:items-center gap-2 p-4 bg-kumfora-cream rounded-xl"
          >
            <dt className="text-body-sm font-medium text-kumfora-charcoal w-full sm:w-auto">
              {key}
            </dt>
            <dd className="text-body text-kumfora-slate font-medium text-kumfora-terracotta">
              {value}
            </dd>
          </div>
        ))}
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="text-center p-8 bg-kumfora-cream rounded-2xl">
            <p className="text-5xl font-display font-bold text-kumfora-terracotta">
              {product.rating}
            </p>
            <div className="flex items-center justify-center gap-1 mt-2">
              {[...Array(5)].map((_, i) => (
                <Star
                  ...
                  className={
                    i < Math.floor(product.rating)
                      ? "fill-kumfora-coral text-kumfora-coral w-6 h-6"
                      : "text-kumfora-lightGray w-6 h-6"
                  }
                />
              ))}
            </div>
          ...
          {reviews.map((review, index) => (
            <article key={index} className="card p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-kumfora-charcoal">
                      {review.name}
                    </span>
                    {review.verified && (
                    ...
                    >
                      {[...Array(5)].map((_, i) => (
                        <Star
                          ...
                          className={
                            i < review.rating
                              ? "fill-kumfora-coral text-kumfora-coral w-4 h-4"
                              : "text-kumfora-lightGray w-4 h-4"
                          }
                        />
                      ))}
                    </div>
                    <time className="text-caption text-kumfora-gray">
              ...
              <h4 className="text-body font-medium text-kumfora-charcoal mt-3">
                {review.title}
              </h4>
  ...
  return (
    <details className="group card overflow-hidden">
      <summary className="flex items-center justify-between p-5 cursor-pointer list-none">
        <span className="text-body font-medium text-kumfora-charcoal pr-4">
          {question}
        </span>
        <ChevronDown
          className={cn(
            "w-5 h-5 text-kumfora-gray transition-transform",
            isOpen && "rotate-180",
          )}
        />
      </summary>
