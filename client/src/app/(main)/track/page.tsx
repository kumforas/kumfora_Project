...
export default function TrackPage() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/orders?orderId=${encodeURIComponent(orderId.trim())}`,
      );
  ...
  return (
    <main className="pt-12 min-h-screen bg-kumfora-cream/30">
      <header className="bg-white border-b border-kumfora-lightGray/50 pt-16 sm:pt-16 pb-8 sm:pb-12">
      ...
      <div className="container-main py-6 sm:py-12 max-w-xl">
        <div className="card p-5 sm:p-8">
          ...
          {error && (
            <div
              className="mt-6 p-4 rounded-xl bg-kumfora-rose/10 border border-kumfora-rose/20"
              role="alert"
            >
              ...
              <div className="space-y-4">
                {TRACKING_STEPS.map((step, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-start gap-4">
                      ...
                      <div>
                        <p
                          className={`text-body font-medium ${isDone ? "text-kumfora-charcoal" : "text-kumfora-gray"}`}
                        >
                          {step.label}
                        </p>
                        {isCurrent && (
                ...
                {result.status === "cancelled" && (
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-body font-medium text-kumfora-rose">
                        Cancelled
                      </p>
