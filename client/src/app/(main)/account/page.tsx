...
interface OrderRecord {
  ...
  items: { product: { id: string; name: string; price: number; thumbnail: string }; quantity: number }[];
  subtotal: number;
  shipping: number;
  total: number;
  status: string;
...
export default function AccountPage() {
  ...
  const { user, loading: authLoading, signOut, updatePassword, deleteAccount } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/auth/orders`);
  ...
  return (
    <main className="pt-16 min-h-screen bg-kumfora-cream/30">
            ...
            <div className="card p-6 space-y-4 sticky top-24">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <h2 className="text-heading-md font-display font-medium text-kumfora-charcoal truncate">{displayName}</h2>
                  <p className="text-body-sm text-kumfora-gray truncate">{user.email}</p>
                </div>
              </div>
              <nav className="space-y-1" role="navigation" aria-label="Account sections">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-body font-medium text-kumfora-rose hover:bg-kumfora-rose/10 transition-colors"
                >
              ...
              <section aria-labelledby="orders-heading">
                ...
                {ordersLoading && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-body text-kumfora-gray">Loading orders...</p>
                  </div>
                )}
                ...
                {ordersError && (
                  <div className="card p-6 text-center py-12">
                    ...
                    <h3 className="text-heading-md font-display font-medium text-kumfora-charcoal mb-2">Error loading orders</h3>
                ...
                {!ordersLoading && !ordersError && orders.length === 0 && (
                  <div className="card p-12 text-center">
                    <Package className="w-16 h-16 text-kumfora-lightGray mx-auto mb-4" />
                    <h3 className="text-heading-md font-display font-medium text-kumfora-charcoal mb-2">No orders yet</h3>
                    <p className="text-body text-kumfora-gray mb-6">Start shopping to see your orders here.</p>
                    <Link href="/shop"><Button leftIcon={<Package className="w-4 h-4" />}>Start Shopping</Button></Link>
                  </div>
                )}
                      ...
                      <article key={order.id} className="card p-6" role="listitem">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="text-heading-md font-display font-medium text-kumfora-charcoal">{order.order_id}</span>
                            <span className="badge bg-kumfora-lightGray text-kumfora-slate">{formatDate(order.created_at)}</span>
                ...
                <div className="space-y-6 max-w-xl">
                  <div className="card p-6">
                    ...
                    <p className="text-body-sm text-kumfora-gray mt-4">
                      Email and phone are managed through your authentication account.
                    </p>
                  </div>

                  <div className="card p-6">
                    <h3 className="text-heading-md font-medium text-kumfora-charcoal mb-4">Change Password</h3>
                    {settingsSuccess && (
                      <div className="mb-4 p-4 rounded-xl bg-kumfora-sageLight/50 border border-kumfora-sage/20">
                        <p className="text-body-sm text-kumfora-sage">Password updated successfully!</p>
                      </div>
                    )}
                    {settingsError && (
                      <div className="mb-4 p-4 rounded-xl bg-kumfora-rose/10 border border-kumfora-rose/20">
                        <p className="text-body-sm text-kumfora-rose">{settingsError}</p>
                      </div>
                    )}
                    <form onSubmit={handlePasswordUpdate} className="space-y-4">
                      ...
                      />
                      <Button type="submit" isLoading={settingsLoading}>Update Password</Button>
                    </form>
                  </div>
                  ...
                  <div className="card p-6 border-kumfora-rose/30">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-4 rounded-xl bg-kumfora-cream/50">
                        <div>
                          <p className="font-medium text-kumfora-charcoal">Sign Out</p>
                          <p className="text-body-sm text-kumfora-gray">Sign out of your account on this device.</p>
                        </div>
                        <Button variant="secondary" onClick={handleSignOut} leftIcon={<LogOut className="w-4 h-4" />}>Sign Out</Button>
                      </div>
                      <div className="flex items-center justify-between p-4 rounded-xl bg-kumfora-rose/5 border border-kumfora-rose/10">
                        <div>
                          <p className="font-medium text-kumfora-charcoal">Delete Account</p>
                          <p className="text-body-sm text-kumfora-gray">Permanently delete your account and all data.</p>
                        </div>
                        <Button variant="danger" onClick={() => setShowDeleteModal(true)} leftIcon={<Trash2 className="w-4 h-4" />}>Delete</Button>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowDeleteModal(false)} role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
          <div className="bg-white rounded-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 id="delete-modal-title" className="text-heading-md font-display font-medium text-kumfora-charcoal">Delete Account</h2>
            </div>
            <p className="text-body text-kumfora-slate mb-2">
              Are you sure you want to permanently delete your account?
            </p>
            <p className="text-body-sm text-kumfora-gray mb-6">
              This action cannot be undone. All your data, orders, and account information will be permanently removed.
            </p>
            {deleteError && (
              <div className="mb-4 p-4 rounded-xl bg-kumfora-rose/10 border border-kumfora-rose/20">
            ...
            <div className="flex gap-3">
              <Button variant="secondary" onClick={() => { setShowDeleteModal(false); setDeleteError(null); }} className="flex-1">Cancel</Button>
              <Button variant="danger" onClick={handleDeleteAccount} isLoading={deleteLoading} className="flex-1">Yes, Delete</Button>
            </div>
          </div>
        </div>
      )}
